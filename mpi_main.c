...
#include <mpi.h>
int      _debug;
double commu_time, bcast_time, gather_time, scatter_time, allreduce_time, reduce_time;
#include "kmeans.h"
...
int main(int argc, char **argv) {
    ...
    commu_time=0; bcast_time=0; gather_time=0; scatter_time=0; allreduce_time=0, reduce_time=0;
    ...
    if (is_output_timing) {
        double max_io_timing, max_clustering_timing,max_commu_time, max_allreduce_time, max_gather_time, max_scatter_time, max_bcast_time,max_reduce_time;
        ...
        MPI_Reduce(&clustering_timing, &max_clustering_timing, 1, MPI_DOUBLE,
                   MPI_MAX, 0, MPI_COMM_WORLD);
        MPI_Reduce(&commu_time, &max_commu_time, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
        MPI_Reduce(&allreduce_time,&max_allreduce_time, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
        ...
        MPI_Reduce(&bcast_time,&max_bcast_time, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
        ...
        if (rank == 0) {
            ...
            printf("Communication timing = %10.4f sec\n", max_commu_time);
            printf("    AllReduce timing    = %10.4f sec\n", max_allreduce_time);
            printf("    Bcast timing        = %10.4f sec\n", max_bcast_time);
            printf("    Gather timing       = %10.4f sec\n", max_gather_time);
            printf("    Scatter timing      = %10.4f sec\n", max_scatter_time);
